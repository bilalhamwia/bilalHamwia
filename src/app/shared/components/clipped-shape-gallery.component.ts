...
@Component({
  selector: 'app-clipped-shape-gallery',
  standalone: true,
  ...
  template: `
    ...
    <section [ngClass]="['grid', columns === 1 ? 'gap-0' : 'gap-8', columnsClass, className]">
      <figure *ngFor="let item of items" [style.clipPath]="'url(#' + item.clipId + ')'" class="relative overflow-hidden group w-full h-full">
        <ng-container *ngIf="item.type === 'image'; else video">
          <img
            [src]="item.src"
            [alt]="item.alt"
            class="transition-all duration-500 aspect-[4/5] min-h-full align-bottom object-cover group-hover:scale-110 w-full h-full"
          />
        </ng-container>
        <ng-template #video>
          <video
            ...
            loop
            playsinline
            class="transition-all duration-500 aspect-[4/5] min-h-full align-bottom object-cover group-hover:scale-110 w-full"
          >
...
export class ClippedShapeGalleryComponent {
